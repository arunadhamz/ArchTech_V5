...
export default function HardwareWorkspace({ project: activeProject }) {
  ...
  const [activeTab, setActiveTab] = useState('hrs'); // 'hrs', 'schematics'
  const [exportFormat, setExportFormat] = useState('md');
  const [terminalOutput, setTerminalOutput] = useState([
    '$ archtech-sys --v3-engine',
    'Ready for hardware deployment extraction.'
  ]);
  ...
  const handleExport = () => {
    const content = activeTab === 'hrs' ? hrsDoc : schemaDoc;
        ...
        <div className={`bg-slate-900 rounded-[32px] overflow-hidden shadow-2xl flex flex-col border border-white/5 ${isFullscreen ? 'fixed inset-4 z-50' : 'flex-1 min-h-0'}`}>
           <div className="px-8 py-4 bg-[#0f172a]/80 flex items-center justify-between border-b border-white/5">
              ...
              <button 
                ...
                disabled={activeTab === 'hrs' ? isGenerating.hrs : isGenerating.schema}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold flex items-center gap-2"
              >
                <Zap size={12} /> {(activeTab === 'hrs' ? isGenerating.hrs : isGenerating.schema) ? 'Generating...' : `Generate ${activeTab.toUpperCase()}`}
              </button>
           </div>
